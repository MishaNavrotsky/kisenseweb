...
export default class Ingredient extends GameObject {
    ...
    constructor(x, y) {
        super(x, y);
        this.dx = 1000;
        loader.sprites["/games/Robocook/images/ingredients/" + this.ingsNames[~~(Math.random() * this.ingsNames.length)] + ".png"].then((img) => {
            this.sprite = img;
        });
    ...
    async init() {
        this.sprite = await loader.sprites["/games/Robocook/images/ingredients/" + this.ingsNames[~~(Math.random() * this.ingsNames.length)] + ".png"];
